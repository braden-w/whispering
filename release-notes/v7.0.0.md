...
**Key Benefits:**
- **Dual Interface**: Every operation provides both reactive (`.options()`) and imperative (`.execute()`) APIs
- **RPC Namespace**: All app operations unified under a single `rpc.*` interface
- **Performance Optimized**: Use lightweight `.execute()` calls when you don't need reactive state
- **Type Safety**: Full TypeScript support with `Result<T, E>` types from the **wellcrafted** library
- **Automatic Caching**: Intelligent caching and background refetching across components

### The Dual Interface Pattern

**Reactive Interface (`.options()`):**
```typescript
// Generates query options for createQuery
const recordings = createQuery(rpc.recordings.getAllRecordings.options());

// This creates a query observer that provides:
// - recordings.data (the actual data)
// - recordings.isPending (loading state)
// - recordings.error (error state)
// - recordings.isStale (cache state)
// - Automatic re-renders on data changes
```

**Imperative Interface (`.execute()`):**
```typescript
// Direct execution without subscriptions
const { data, error } = await rpc.recordings.deleteRecording.execute(recordingId);

// This provides:
// - Lightweight, no query observers or subscriptions
// - Better performance for one-time operations
// - Perfect for event handlers and workflows
// - Works outside component lifecycle
```
...
const recordings = createQuery(rpc.recordings.getAllRecordings.options());
// Gets: isPending, data, error, isStale automatically
