...
Major highlights:
• Complete TanStack Query integration for reactive data management
• Service layer rewrite with platform abstractions (desktop/web/extension)
• Shadcn-svelte UI migration for modern, accessible components
• Enhanced transcription providers with better error handling
• Improved keyboard shortcuts with conflict resolution
• VAD and CPAL recording modes for better audio capture

⚠️ Breaking changes - automatic migration included for settings and database
<!-- SUMMARY:END -->
...
**Note**: We're using a custom fork of TanStack Query optimized for Svelte 5 compatibility and enhanced Result type handling.
