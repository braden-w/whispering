<script lang="ts">
  ...
  function onChangeShortcutClick() {
    toast.promise(registerShortcut(currentGlobalShortcut, toggleRecording), {
      ...
      error:
        "Couldn't register shortcut. The shortcut code must be a valid keyboard shortcut."
    })
  }
  ...
  onMount(
    async () => await registerShortcut(currentGlobalShortcut, toggleRecording)
  )
